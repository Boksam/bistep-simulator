"""Water temperature simulation model.

This module contains the WaterTemperatureSimulator class, which is responsible
for analyzing historical water temperature data and generating future synthetic
...
class WaterTemperatureSimulator:
    """Simulates future water temperature data based on historical patterns.

    This class decomposes a time series of water temperature into trend,
    ...
    def _handle_outliers(
        ...
        1.  Absolute threshold: Values exceeding this are considered outliers.
        2.  Rolling window: Values outside a `sigma` standard deviation range
            from the rolling median are considered outliers.

        Args:
            series: The input time series data.
            window: The size of the rolling window.
            sigma: The number of standard deviations for outlier detection.
            absolute_threshold: The absolute temperature threshold.
        ...
        print(f"  - Found {total_outliers.sum()} outliers.")
        series_no_outliers[total_outliers] = np.nan
        return series_no_outliers

    def load_and_prepare_data(self, csv_path: str) -> pd.Series:
        """Loads, preprocesses, and interpolates water temperature data.

        Assumes the CSV has "timestamp" and "temperature" columns.
        ...
        try:
            ...
            series.name = "hourly_avg_water_temp"
    ...
    def _decompose_with_stl(
        self, series: pd.Series, period: int = 365
    ...
    def model_yearly_seasonality(
        self, series: pd.Series, period: int = 365, model: str = "additive"
    ) -> pd.Series:
    ...
    def model_daily_seasonality_and_noise(
        self, series: pd.Series, period: int = 24, model: str = "additive"
    ):
    ...
    def generate_yearly_seasonality(
        self, datetime_range: pd.DatetimeIndex
    ) -> np.ndarray:
    ...
    def generate_noise(self, n_samples: int, seed: Optional[int] = None) -> np.ndarray:
        ...
        # If seed is None, this re-seeds the generator from OS entropy (e.g., /dev/urandom).
        # This ensures stochastic results for each API call when no seed is specified.
        np.random.seed(seed)
    ...
    def simulate(
        ...
        """Generates synthetic water temperature data for a given period.
        ...
        Returns:
            A pandas Series containing the simulated water temperature data.
        """
        ...
        return pd.Series(simulated_temp, index=datetime_range, name="simulated_temp")
    ...
    def run_analysis(
        ...
        Returns:
            The fitted WaterTemperatureSimulator instance.
        """
